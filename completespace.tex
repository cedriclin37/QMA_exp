...
Our algorithm (Theorem \ref{thm: matrix inversion alg}) actually approximates the matrix entry $|H^{-1}(s,t)|$ to precision $2^{-\mathcal{O}(k)}$. In particular, if we take $k = \bigoh(\log n)$ we get the following:
\begin{corollary}
...
This result improves upon Ta-Shma's result \cite{tashma} in two ways. First, unlike Ta-Shma's algorithm, our algorithm does not use intermediate measurements.  Although the ``safe storage'' principle of quantum time-bounded computation tells us that quantum measurements can be deferred, the standard method of deferring quantum measurements may incur an exponential blow-up in space.% an equivalent principle is not known for space complexity.  The standard procedure for deferring measurements in time-bounded computation may incur an exponential blow-up in space.  
...
\subsection{Minimum Eigenvalue problem} 
...
$\bigoh(\qca{k(n))}$ is $\unitaryBQSPACE{k(n)}$-hard under classical $\poly(n)$, $\bigoh(k(n))$ space reductions.
\end{lemma}
