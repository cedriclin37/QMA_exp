...
\newcommand\Logspace{{\sf{Logspace}}}
...
\newcommand\preciseilh{\textit{Precise Local Hamiltonian}}
\newcommand\matrixinvert[1]{{#1}\textit{-Well-conditioned Matrix Inversion}}
...
%+Title
\title{A Complete Characterization of Unitary Quantum Space Complexity}
...
\maketitle
\begin{abstract}
We give two complete characterizations of unitary quantum space complexity.  The first is based on the Matrix Inversion problem.  We show that given a succinct $2^{k(n)} \times 2^{k(n)}$ matrix $A$, approximating a particular entry of $A^{-1}$ is complete for the class of problems solvable by a quantum algorithm that uses $k(n)$ space and performs all measurements at the end of the computation.  We then show that the problem of deciding the ground state energy of a sparse Hamiltonian, whose entries are efficiently-specified, gives a second characterization of unitary quantum space complexity.  As a consequence of this, we establish that $\QMA$ with exponentially small completeness-soundness gap is equal to $\PSPACE$, that determining whether a local Hamiltonian is frustration-free is $\PSPACE$-complete, and give a provable setting in which the ability to prepare $PEPS$ states gives less computational power than the ability to prepare the ground state of a generic local Hamiltonian.
\end{abstract}

\section{Introduction}
Matrix Inversion is one of the most important and ubiquitous computational problems.  It is complete for $\DET$, the class of functions $\Logspace$-reducible to computing the determinant of an integer matrix, which can be computed in deterministic $\bigoh(\log^2{n})$ space \cite{berkowitz, cook}.  It has long been a major question in computational complexity to understand if Matrix Inversion can be solved in $\Logspace$, i.e., in deterministic space $\bigoh(\log{n})$, which would imply that $\Logspace=\DET$.

Recently, Ta-Shma, building on work of Harrow, Hassidim, and Lloyd, showed that inverting a well-conditioned matrix can be approximated by a quantum algorithm using a logarithmic amount of space \cite{tashma,HHL}.  This gives a quadratic speedup in space over the best known classical algorithms.  

Our first result shows that the matrix inversion problem characterizes \emph{unitary} quantum space complexity.  In particular, we prove that given a well-conditioned succinct $2^{k(n)}\times 2^{k(n)}$ matrix $A$ and the index of a desired entry, this entry of $A^{-1}$ can be estimated in unitary Quantum space $\bigoh(k(n))$.  In so doing, we improve upon prior results in two ways. First, unlike Ta Shma's algorithm, our algorithm does not use intermediate measurements.  This is important in quantum space complexity, where it is not known if measurements can be efficiently deferred.  In addition, we show that matrix inversion  is hard for unitary quantum space $k(n)$ under classical $k(n)$ space reductions.  When $k(n)=\bigoh(\log{n})$ this result has an interesting interpretation: one cannot show that Matrix Inversion is solvable in classical $\Logspace$ unless one also shows that classical $\Logspace$ equals unitary quantum $\Logspace$, which seems quite unlikely.

Our second result concerns a complete characterization of quantum space complexity via the ``sparse Hamiltonian problem'': Given as input a succinct  $2^{k(n)}\times 2^{k(n)}$ Hermitian matrix and two real parameters $a,b$ so that $b-a\geq 2^{-\bigoh(k(n))}$, is the minimum eigenvalue less than $a$ or at least $b$?  We prove that this problem is complete under $k(n)$ space reduction for the class of problems solvable by a unitary bounded error quantum algorithm using space $k(n)$.

This second characterization has several interesting consequences.  In particular, we use it to prove that $\QMAexp$, the variant of $\QMA$ with exponentially small completeness-soundness gap, contains $\BQPSPACE=\PSPACE$, and combining this with a matching upper bound, we obtain $\QMAexp=\PSPACE$.  The closest classical counterpart of $\QMAexp$ is $\NP^{\PP}$: given a classical witness, the verifier runs a classical computation that in the YES case accepts with probability at least $c$, or in the NO case accepts with probability at most $s$, where $c>s$. Note that in the classical case $c - s > \exp(\poly)$ is automatically satisfied. Since $\NP^{\PP}$ is in the counting hierarchy, the entirety of which is contained in $\PSPACE$ (see e.g., \cite{allenderwagner}), we see that the quantum proof protocol is strictly stronger than the classical one, unless the counting hierarchy collapses to the second level.
...
On the other hand, suppose we have an oracle $O_{LH}$ that given the description of a local Hamiltonian, outputs its unique ground state\footnote{If the ground space is degenerate, we can always slightly perturb the Hamiltonian to make it non-degenerate.}. Then our results show that $\QMAexp = \PSPACE \subseteq \PQP^{O_{LH}}_{\parallel,\text{classical}}$. This shows that, at least in the setting of unbounded-error quantum computation, PEPS do not capture the full computational complexity of general local Hamiltonian ground states unless $\PP=\PSPACE$. We leave open the problem of determining the complexity of $\BQP^{O_{LH}}_{\parallel,\text{classical}}$.
...
A \emph{quantum circuit} consists of a series of quantum gates each of which come from some universal gateset, such as the gateset consisting of Hadamard and Toffoli gates \cite{shi}.  A \emph{description} of the quantum circuit is a string consisting of the gates in the circuit, together with the qubits that each gate acts on.
 For a function $f:\mathbb{N}\rightarrow\mathbb{N}$, we say a family of quantum circuits $\{Q_x\}_{x\in\{0,1\}^n}$ is \emph{$f$ space uniformly generated} if there exists a deterministic classical Turing machine that on input $x\in\{0,1\}^n$ uses space $g(n)$ for some function $g=\bigoh(f(n))$ and outputs the description of $Q_x$. Unless otherwise specified, we will assume that this machine runs in polynomial time.\footnote{In fact this additional constraint of polynomial time can be assumed without lost generality \wf{More needed...}}  Likewise a family of quantum circuits $\{Q_x\}_{x\in\{0,1\}^n}$ is \emph{$f$-time uniformly generated} if there exists a deterministic classical Turing machine that on input $x\in\{0,1\}^n$ runs in time $g(n)$ for some function $g=\bigoh(f(n))$ and outputs the description of $Q_x$.  
 %Bill: I decided not to add time and space constructibility, feel free to uncomment if you think it's necessary
 %Furthermore, we say a function $s:\mathbb{N}\rightarrow\mathbb{N}$ is \emph{space} (resp. \emph{time}) \emph{constructible} if there is a classical Turing machine that, on input $1^n$, outputs $s(n)$ using space $\bigoh(s(n))$ (resp. computes $t(n)$ in time $\bigoh(t(n))$).
\subsection{Space-bounded computation}
We start with the definitions of classical bounded space computation.  For $k:\mathbb{N}\rightarrow\mathbb{N}$, a function $f:\{0,1\}^{*}\rightarrow\{0,1\}^*$ is said to be in $k(n)$ space if any bit of $f(x)$ can be computed by a deterministic Turing machine using space $\bigoh(k(|x|))$.  Consequently, $\Logspace$ is the class of functions that can be computed in $\log{n}$ space.

We now define space-bounded quantum complexity classes:
\wf{Should we define succinct here?}
\begin{definition}
We say a promise problem $L=(L_{YES},L_{NO})$ is in $\QSPACE[k(n)](c,s)$ if it is there exists a $k$ space uniformly generated family of quantum circuits $\{Q_x\}_{x\in\{0,1\}^n}$, where each circuit $Q_x$ acts on $k(|x|)$ qubits so that:\\
 If $x \in L_{yes}$:
\begin{equation}
...
For the purposes of this work, we will demand that the space bound $k(n)$ is at least $\bigoh(\log{n})$ and no more than $2^{\poly(n)}$.  Further we demand that the size of each circuit $Q_x$ is at most exponential in $k(|x|)$... \wf{This needs explaining...}
\end{definition}
...
\begin{definition}We say a promise problem $L=(L_{yes},L_{no})$ is in $\bddQMA{t}{k}{m}{c}{s}$ if there exists a $t$ time and $(k+m)$ space uniformly generated family of quantum circuits $\{ V_x\}_{x\in\{0,1\}^n}$, each of size at most $t(|x|)$, acting on $k(|x|)+m(|x|)$ qubits, so that:\\
...
\end{definition}
Finally, we will frequently be assuming the existence of  algorithms that compute algebraic functions on $n$-bit numbers, as such as $\sin,\cos,\arcsin,\arccos$ and exponentiation, to within inverse polynomial accuracy in classical $O(\log{n})$ space.  Algorithms for these tasks have been designed by Reif \cite{reif}.
...
Suppose we are given the encoding of a succinct sparse $2^{k(n)} \times 2^{k(n)}$ PSD matrix $A$ and an upper bound $\kappa$ on the condition number of $A$, such that $\kappa^{-1} I \preceq A \preceq I$. We are also given $\poly(n)$-time $\mathcal{O}(k\log(\kappa/\epsilon))$-space uniform quantum circuits $U_a$ and $U_b$ acting on $k$ qubits. Let $U_a\ket{0}^{\otimes k(n)} = \ket{a}$ and $U_b\ket{0}^{\otimes k(n)} = \ket{b}$. The following tasks can be performed with $\poly(n)$-time $O(k\log(\kappa/\epsilon))$-space uniform quantum circuits over $O(k\log(\kappa/\epsilon))$ qubits:
\begin{enumerate}
\item With at least constant probability, output the state $A^{-1}\ket{b} / \|A^{-1}\ket{b}\|$ up to precision $\epsilon$.
\item Estimate $\|A^{-1}\ket{b}\|$ up to precision $\epsilon$.
\item Estimate  $|\bra{a}A^{-1}\ket{b}|$ up to precision $\epsilon$.
\end{enumerate}
\wf{TODO: Define the third as Well-conditioned k(n)-Matrix Inversion}
These circuits do not require intermediate measurements.
\end{theorem}
Note that if intermediate measurements were allowed instead, then in the case $k = \mathcal{O}(\log n)$ this result is already proven by Ta-Shma \cite[Theorem~6.3]{tashma}. We strengthen Ta-Shma's result in the logspace case by showing that intermediate measurements are not necessary. \wf{We should also point out that our result scales for non-logarithmic space bounds, where Ta-Shma's does not}

We first start out with the following lemma:
\begin{lemma}[Ta-Shma \cite{tashma}] \label{lem: matrix inversion lemma}
There is a $O(k\log(\kappa/\epsilon'))$-space uniform quantum unitary transformation $W_A$ over $O(k\log(\kappa/\epsilon'))$ qubits, such that for any $k$-qubit input state $\ket{b}$,
\begin{equation}
W_A (\ket{0}^{\otimes \ell} \otimes \ket{b}) = \alpha \ket{0}_{out} \otimes \ket{\psi_b} + \sqrt{1-\alpha^2} \ket{1}_{out} \otimes\ket{junk_b},
\end{equation}
...
\end{lemma}
The proof of this lemma is essentially contained in the proof of \cite[Theorem~6.3]{tashma}, or can be obtained by combining \cite[Theorem~4.1]{tashma} with the analysis of Harrow, Hassidim and Lloyd \cite{HHL}.
...
We will now show an alternate characterization of unitary quantum space via another complete problem for $\unitaryBQSPACE{k(n)}$, for any [TODO: space constructible?] function $k(n)$. Here completeness is defined under a reduction computable by a \emph{classical} algorithm in polynomial time and $\mathcal{O}(k(n))$ space.
\begin{definition}
Let $M$ be a $2^{k(n)} \times 2^{k(n)}$ matrix, where $n$ is the input size. Say that $M$ is a \emph{sparse} matrix if each row has at most polynomially many nonzero entries. Moreover if $M$ is sparse, we say it is \emph{succinct} matrix if there is a (uniformly generated) circuit, \emph{the succinct encoding}, that outputs the nonzero entries of any given row in $\poly(n)$ time and $\mathcal{O}(k)$ space.
\wf{Should we have defined succinct before this?}
\end{definition}
\begin{definition}[$\spechamiltonian{k(n)}$ problem]
Given as input is the succinct encoding of a succinctly representable sparse $2^{\mathcal{O}(k(n))} \times 2^{\mathcal{O}(k(n))}$ Hermitian positive semidefinite (PSD) matrix $H$. Let $\lambda_{min}$ be the minimum eigenvalue of $H$. It is promised that either $\lambda_{min} \le a$ or $\lambda_{min} \ge b$, where $a$ and $b$ are some real numbers such that $b-a > 2^{-\mathcal{O}(k(n))}$. Output 1 if $\lambda_{min} \le a$, and output 0 otherwise.
\end{definition}
...
%%HERE
%\subsubsection {In-place gap amplification of $\QMA$ with phase estimation}
...
\section{Achieving Perfect Completeness}\label{app:perfectcompleteness}
