...
Recently, Ta-Shma \cite{tashma}, building on work of Harrow, Hassidim, and Lloyd \cite{HHL}, showed that a well-conditioned $n \times n$ matrix can be approximately inverted by a quantum $O(\log n)$ space algorithm, but with intermediate measurements. Similarly, Ta-Shma also gives an algorithm for computing eigenvalues of a Hermitian matrix with similar space. These algorithms achieve a quadratic advantage in space over the best known classical algorithms, which require $\Omega(\log^2n)$ space.  This is the maximum quantum advantage possible, since Watrous has shown $\BQSPACE[k(n)]\subseteq\DSPACE[\bigoh(k(n)^2)]$ \cite{Watrous99,Watrous03} even for quantum algorithms with intermediate measurements.

We note that our results improve upon Ta-Shma's results. For example, a corollary of our completeness result for matrix inversion, and noticing that our algorithm for matrix inversion (Theorem \ref{thm: matrix inversion alg}) actually gives a high-precision approximation, gives the following corollary.
\begin{corollary}
...
This corollary (and a similar one for computing the minimum eigenvalue) improves upon Ta-Shma's result \cite{tashma} in two ways. First, unlike Ta-Shma's result, we show that these problems can be solved without intermediate measurements - note that the standard method of deferring quantum measurements may incur an exponential blow-up in space.  In addition, we show that the problems of inverting well-conditioned matrices and computing minimum eigenvalues of Hermitian matrices are hard for unitary quantum logspace under $\Logspace$-reductions. In other words, one cannot show that well-conditioned matrices are invertible in $\Logspace$ unless one also shows that $\Logspace=\unitaryBQL$, which seems quite unlikely. Interestingly, our algorithm for computing the minimum eigenvalue is very different from Ta-Shma's algorithm.
...
\subsection{Statements of results and proof summary} \label{sec: results}
...
A consequence of the proof is the following theorem, of which Corollary \ref{cor: preciseqma} is a special case:
...
\end{proof}
\begin{theorem}
$\matrixinvert{\bigoh(k(n))}$ is $\unitaryBQSPACE{k(n)}$-hard under classical reductions computable in polynomial time and $\bigoh (k)$ space.
\end{theorem}
\begin{proof}
...
\subsection{$\spechamiltonian{\bigoh(k(n))}$ is hard for $\unitaryBQSPACE{k(n)}$} \label{sec: specham-hardness}
