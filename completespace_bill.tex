...
\begin{corollary}
The problem of approximating, to $2^{-\mathcal{O}(n)}$ precision, an entry of the inverse of an $n \times n$ matrix with condition number at most $2^{\mathcal{O}(n)}$ is $\unitaryBQL$-complete under $\Logspace$-reductions.
\end{corollary}
...
\wf{It's already defined above, which to keep?  In fact, do we need this section}
