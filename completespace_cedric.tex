...
Matrix Inversion is one of the most important and ubiquitous computational problems.  The problem of inverting integer matrices is complete for $\DET$, the class of functions $\Logspace$-reducible to computing the determinant of an integer matrix, which can be computed in deterministic $\bigoh(\log^2{n})$ space \cite{berkowitz, cook}.  It has long been a major question in computational complexity to understand if Matrix Inversion can be solved in $\Logspace$, i.e., in deterministic space $\bigoh(\log{n})$, which would imply that $\Logspace=\DET$.
...
\begin{definition}[\matrixinvert{k(n)}]
	Given as input is an $2^{k(n)} \times 2^{k(n)}$ succinct PSD matrix $H$ with a known upper bound $\kappa = 2^{\mathcal{O}(k(n))}$ on the condition number, so that $\kappa^{-1}I\preceq H \preceq I$, and $s,t\in \lbrace 0,1\rbrace^{k(n)}$. It is promised that either $|\langle s|H^{-1}|t\rangle|\geq b$
...
Here the subscript $\Unitary$ means that our quantum computation is unitary: there are no intermediate measurements. We also note that our algorithm (Theorem \ref{thm: matrix inversion alg}) actually approximates the matrix entry $|\bra{s}H^{-1}\ket{t}|$ to precision $2^{-\mathcal{O}(k)}$. In particular, if we take $k = \log n$ we get the following:
\begin{corollary}
The problem of approximating, to $2^{-\mathcal{O}(n)}$ precision, an entry of the inverse of an $n \times n$ matrix with condition number at most $2^{\mathcal{O}(n)}$ is $\unitaryBQL$-complete under $L$-reductions.
\end{corollary}

This result improves upon Ta-Shma's result \cite{tashma} in two ways. First, unlike Ta-Shma's algorithm, our algorithm does not use intermediate measurements.  This is important in quantum space complexity, where it is not known if measurements can be efficiently deferred.  In addition, we show that the problem of inverting well-conditioned matrices is hard for unitary quantum logspace under $L$-reductions. In other words, one cannot show that well-conditioned matrices are invertible in solvable in classical $\Logspace$ unless one also shows that classical $\Logspace$ equals unitary quantum $\Logspace$, which seems quite unlikely.
...
\subsection{Sparse Hamiltonian problem}
Our second result concerns a complete characterization of quantum space complexity via the following problem:
\begin{definition}[$\spechamiltonian{k(n)}$ problem]
Given as input is %the succinct encoding of 
a succinct $2^{\mathcal{O}(k(n))} \times 2^{\mathcal{O}(k(n))}$ positive semidefinite (PSD) matrix $H$. Let $\lambda_{min}$ be the minimum eigenvalue of $H$. It is promised that either $\lambda_{min} \le a$ or $\lambda_{min} \ge b$, where $a(n)$ and $b(n)$ are real functions constructible in classical $\mathcal{O}(k(n))$ space such that $b-a > 2^{-\mathcal{O}(k(n))}$. Output 1 if $\lambda_{min} \le a$, and output 0 otherwise.
\end{definition}
...
\begin{definition}
We say a promise problem $L=(L_{YES},L_{NO})$ is in $\QSPACE[k(n)](c,s)$ if there exists a $\poly(n)$-time $\mathcal{O}(k)$-space uniformly generated family of quantum circuits $\{Q_x\}_{x\in\{0,1\}^n}$, where each circuit $Q_x=U_{x,T}U_{x,T-1}\cdots U_{x,1}$ has $T=2^{\mathcal{O}(k)}$ gates, and acts on $\mathcal{O}(k(|x|))$ qubits, such that:\\
 If $x \in L_{yes}$:
\begin{equation}
