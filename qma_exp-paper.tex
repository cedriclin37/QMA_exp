...
Suppose $A$ is a $2^n \times 2^n$ symmetric and succinctly representable sparse matrix, with at most $d$ nonzero entries in each row. Then treated as a Hamiltonian, the time evolution $\exp(-iAt)$ can be simulated using $\text{\emph{poly}}(n,d,\|A\|, t) \cdot \log(1/\epsilon)$ operations.
\end{theorem}
The crucial thing to notice in Theorem \ref{thm:ham_sim} is the logarithmic scaling in the error $\epsilon$; this implies that we can obtain exponential precision in $\exp(-iAt)$ using only polynomially many operations. Also note that we can upper bound $\|A\|$ with the following observation:
\begin{remark}
...
\end{equation}
for some polynomial $g'(n)$ depending on the problem, and any $\epsilon = 2^{-\poly(n)}$ of our choice. The number of operations for that protocol is upper bounded by $T \le h(n) (1+\log (1/\epsilon))$ for some polynomial $h(n)$ independent of $\epsilon$. Now we can pick $\epsilon$ to be a small enough inverse exponential function such that
\begin{equation}
\epsilon (T^2+1) \le \epsilon [h^2(n) (1+\log(1/\epsilon))^2 +1] < 2^{-g'(n)} 
\end{equation}
