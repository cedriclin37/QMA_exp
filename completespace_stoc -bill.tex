...
\newcommand\spechamiltonian[1]{#1\textit{-Minimum Eigenvalue}}
...
This corollary (and a similar one for computing the minimum eigenvalue) improves upon Ta-Shma's result \cite{tashma} in two ways. First our algorithms  solve these problems without needing intermediate measurements.  Unlike in time complexity, where the ``Principle of safe storage'' gives a procedure to time efficiently defer intermediate measurements, these methods may incur an exponential blow-up in space.  

Of course one might wonder why we care so much about avoiding intermediate measurements.  Principally, removing intermediate measurements from the computation allows us to give matching hardness results, showing the optimality of our algorithms.  This is the second way our results improve on those of Ta-Shma.  In particular, our proofs will crucially use space-efficient methods for the amplification of unitary quantum computations, which simply are not known in the non-unitary model.  This is because the techniques require applying the inverse of the circuit, which of course is impossible if the circuit contains intermediate measurements.  We will also rely on ideas from Kitaev's clock construction, which constructs a local Hamiltonian from a unitary quantum circuit. 

Specifically, we will show that the problems of inverting well-conditioned matrices and computing minimum eigenvalues of Hermitian matrices are hard for unitary quantum logspace under $\Logspace$-reductions. In the case of our algorithm for Matrix Inversion, this means that the upper bound on the condition number bound is unlikely to be improved upon.  Likewise, this gives some of the strongest evidence that the general Matrix inversion problem cannot be solved in $\Logspace$, since otherwise our results would immediately imply $\Logspace=\unitaryBQL$, which seems quite unlikely. 
...
Lastly, we are able to strengthen our characterization to show that $\preciseQMA$ contains $\PSPACE$ (see Appendix \ref{app:perfectcompleteness}), even when restricted to having perfect completeness.  This allows us to prove that testing if a local Hamiltonian is frustration-free is a $\PSPACE$-complete problem (Appendix \ref{app:localhamiltonian}).
...
The first problem that characterizes unitary quantum space is the $\spechamiltonian{k(n)}$ problem:
...
\end{theorem}
\begin{proof}
First we describe the upper-bound, proving containment in $\unitaryBQSPACE{k(n)}$.  The first step is to show that the $\spechamiltonian{k(n)}$ problem is in $\preciseQMA$ with a unitary verifier using $k(n)$ qubits of space, or more formally: 
...
We now have a $k(n)$-space bounded $\preciseQMA$ protocol that uses a $k(n)$ qubit witness.  The next step is to amplify this protocol.  To do this we need to develop new {\emph{space-efficient}} $\QMA$ error amplification procedures.  We start by extending ideas used in the ``in-place'' $\QMA$ amplification procedure of Marriott and Watrous \cite{mw05}.  The Marriott and Watrous procedure does not require Merlin to send additional copies of his state.  However, the analysis still uses a Chernoff bound to reduce error, and so requires repeating the protocol $\bigoh(r/(c-s)^2)$ times to amplify a procedure with completeness $c$ and soundness $s$ to one with completeness $1-2^{-r}$ and soundness $2^{-r}$.  Recording the outcomes of each repetition uses an additional $r/(c-s)^2$ qubits of space.  Nagaj, Wocjan, and Zhang show that this can be improved using a phase estimation based procedure, to obtain the same completeness and soundness with only an additional $r\log{\frac{1}{c-s}}$ qubits \cite{nwz11}.  We manage to achieve these error bounds while using only $r+\log{\frac{1}{c-s}}$ additional qubits, which is needed for our result\footnote{Indeed, in recent work we were able to improve this result to achieve such amplification using only $\log{\frac{r}{c-s}}$ additional qubits of space \cite{fklmn16}.}. See Appendices \ref{app: space efficient amplification} and \ref{app: gap amplification} for more details.  

We apply these techniques to our space bounded $\preciseQMA$ proctocol to obtain a new $\QMA$ protocol that uses the same amount of space but with completeness and soundness error exponentially small in the witness length, $k(n)$.

Once we have this protocol we can consider the same verification procedure run on the maximally mixed state witness.  Equivalently, we can think of this procedure as running the same verification procedure with a uniformly chosen classical witness.  This protocol maintains the same space complexity, but doesn't require Merlin to send a witness.  We show that the resulting completeness and soundness bounds are now separated by $2^{-k(n)}$, which allows us to again amplify our protocol using our space-efficient amplification technique.  The time used by the resulting bounded error protocol is exponential in $k$, but crucially uses only $O(k(n))$ qubits of space, establishing our upper bound.  See Appendix \ref{app: removingwitness} for the details.

The lower bound, showing $\BQSPACE[k(n)]$-hardness, is an application of Kitaev's clock construction \cite{ksv02} and our new space-efficient amplification procedure.  Using our uniformity condition, it is easy to see that any language in $\BQSPACE[k(n)]$ can be decided with a quantum circuit of size at most $2^{k(n)}$.  We first use our space-efficient  amplification procedure to reduce the error.  Then following Kitaev, we construct a Hamiltonian out of the amplified circuit.  It is straightforward to see that the resulting Hamiltonian is sparse and succinct, and that the ground state energy is inverse-exponentially lower in the case the circuit accepts than in the case it rejects.  The details are in Appendix \ref{sec: specham-hardness}.
...
A consequence of our proof is the following theorem, of which Corollary \ref{cor: preciseqma} is a special case:
...
We first briefly summarize the algorithm of Ta-Shma \cite{tashma}, which  is based on the linear system solving algorithm of Harrow, Hassidim and Lloyd \cite{HHL}.  In our language, Ta-Shma shows that $\matrixinvert{\bigoh(\log{n})}$ can be solved by a quantum logspace algorithm that uses intermediate measurements. This corresponds to inverting an $n \times n$ matrix with condition number at most $\poly(n)$.
...
\end{lemma}
\begin{proof}
The proof of this lemma is implicit in the proof of \cite[Theorem~6.3]{tashma}, or can be obtained by combining \cite[Theorem~4.1]{tashma} with the analysis of Harrow, Hassidim and Lloyd \cite{HHL}. The essential idea of the proof is to use phase estimation on $\exp(iH)$ to compute the eigenvalues $\lambda$ of the matrix $H$ into an auxilliary register; implement the unitary transformation $\ket{\lambda}\ket{0} \rightarrow \ket{\lambda}[(\kappa\lambda)^{-1}\ket{0}+(\sqrt{1-(\kappa\lambda)^{-2}}\ket{1}]$; and finally uncompute the eigenvalues $\lambda$. Ta-Shma showed how to implement $\exp(iH)$ in $\bigoh (k+\log(1/\epsilon))$ space \cite[Theorem~4.1]{tashma} (their proof works for general matrices with efficient encodings); alternatively the more recent sparse Hamiltonian simulation algorithms by Berry et al. (Theorem \ref{thm:ham_sim}) has the same space requirement and is more time efficient.
\end{proof}
Intuitively, Lemma \ref{lem: matrix inversion lemma} tells us that there exists a space efficient quantum algorithm that produces a state proportional to $H^{-1}\ket{b}$ with probability $1/\poly$.  Of course our goal is to produce a state with constant overlap to the state $\ket{0}_{out} \otimes \ket{\psi_b}$ together with an estimate for $\alpha \approx \|H^{-1}\ket{b}\|$.  From here our algorithm differs from that of Ta-Shma and uses a combination of amplitude amplification and phase estimation.  This completely sidesteps both the somewhat complicated analysis and intermediate measurements of Ta-Shma's algorithm.  It is straightforward to further extend this algorithm to the case where $k(n) = \omega(\log(n))$.
...
\end{equation}
$\Pi_0$ projects onto the initial subspace, while $\Pi_1$ projects onto the initial states that would be accepted by the final measurement. By an argument similar to Grover's algorithm, if we implement the rotation $R=-(I-2\Pi_0)(I-2\Pi_1)$ $\bigoh (1/\alpha)$ times we will generate an output state with constant overlap with $\ket{0}_{out} \otimes \ket{\psi_b}$. $\alpha$ can be estimated by performing phase estimation on the operator $R$. \wf{Should this be clarified to satisfy the referee?  I'm not sure that's necessary...}
...
We establish that $\matrixinvert{k(n)}$ is $\unitaryBQSPACE{\mathcal{O}(k)}$-hard using a similar argument to Harrow, Hassidim, and Lloyd \cite{HHL}, in which given a quantum circuit acting on $k(n)$ qubits we can construct a succinctly described well-conditioned $2^{\mathcal{O}(k)}\times 2^{\mathcal{O}(k)}$ matrix $H$, so that a single element of $H^{-1}$ is proportional to the success probability of the circuit.  This establishes our proof.  See Appendix \ref{app: matrixinversion-hardness} for a detailed proof. 
...
\begin{align}
\alpha &\ge \frac{\|H^{-1}\ket{b}\|}{\kappa} - \epsilon' \\
...
We begin our hardness proof by considering the following simple hard problem for $\unitaryBQSPACE{k(n)}$:
\begin{definition}[\qca{k(n)}]
	Given as input is the description of a quantum circuit $Q$ acting on $k(n)$ qubits with $T = 2^{\mathcal{O}(k(n))}$ 1- or 2-qubit gates.  It is promised that either the matrix entry $|\langle {\zero}|Q|{\zero}\rangle| \geq 2/3$ or $|\langle {\zero}|Q|{\zero}\rangle| \leq 1/3$; determine which is the case.
\end{definition}
...
\section{$\spechamiltonian{k(n)}\in\unitaryBQSPACE{k(n)}$ proof details}
...
\end{proof}
Notice that Theorems \ref{thm: spechamiltonian} and \ref{thm: equivalence} follow from Theorems \ref{lem:qma protocol}, \ref{thm: pspace upper bound}, and Theorem \ref{thm: specham-hardness}. In particular, considering the polynomial space case in Theorem \ref{thm: equivalence} proves Corollary \ref{cor: preciseqma}, that $\preciseQMA=\PSPACE$.
...
%\end{theorem}
We now consider the problem of achieving prefect completeness for $\preciseQMA$. Specifically, we will show the following:
\begingroup
