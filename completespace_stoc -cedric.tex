...
Our first characterization of unitary quantum space is based on the following problem:
...
We only give a high level overview of the proof here; for the complete proof see Appendix \ref{app: pspace upper bound}. The core of the proof is to develop and use new {\emph{space-efficient}} $\QMA$ error reduction procedures. Our procedures are based on the ``in-place'' $\QMA$ amplification procedure of Marriott and Watrous \cite{mw05}, which allows the error in a $\QMA$ proof system to be reduced without requiring Merlin to send additional copies of the witness state. The improved phase-estimation based procedure of Nagaj, Wocjan, and Zhang \cite{nwz11} gives a procedure that reduces the error to $2^{-r}$ using only $\bigoh\left(r+\log{\frac{1}{c-s}}\right)$ additional qubits and $r/(c-s)$ repetitions of the circuit and its inverse. We derive a procedure (Lemma \ref{lem: gap amp}) that gives the same error bounds while using only $\bigoh\left(r+\log{\frac{1}{c-s}}\right)$ additional qubits, but still using only $r/(c-s)$ repetitions of the circuit; the improved space bound will be required for our purposes\footnote{Indeed, in recent work we were able to improve this result to achieve such amplification using only $\log{\frac{r}{c-s}}$ additional qubits of space \cite{fklmn16}.}.
...
Our second result concerns the $\QMA$-complete Local Hamiltonian problem. We show that the variant of the \emph{local} Hamiltonian problem, where the promise gap is only exponentially small, is also $\PSPACE$-complete. 
...
There is a $\bigoh (k + \log(\kappa/\epsilon'))$-space uniform quantum unitary transformation $W_H$ over $k+\ell = \bigoh (k+ \log(\kappa/\epsilon'))$ qubits and using $\poly(k,\kappa/\epsilon')$ gates, such that for any $k$-qubit input state $\ket{b}$,
\begin{equation}
...
\end{enumerate}
An appropriate error analysis of this procedure is the technical bulk of the proof; we direct the reader to \cite{HHL}. For Step 1, Ta-Shma showed how to implement $\exp(iH)$ in $\bigoh (k+\log(1/\epsilon))$ space \cite[Theorem~4.1]{tashma} (their proof works for general matrices with efficient encodings); recent sparse Hamiltonian simulation algorithms (Theorem \ref{thm:ham_sim}) give a time efficient way to do this.
\end{proof}
Intuitively, Lemma \ref{lem: matrix inversion lemma} tells us that there exists a space efficient quantum algorithm that produces a state proportional to $H^{-1}\ket{b}$ with probability at least $1/\kappa$.  Of course our goal is to produce a state with constant overlap to the state $\ket{0}_{out} \otimes \ket{\psi_b}$ together with an estimate for $\alpha \approx \|H^{-1}\ket{b}\|$.  From here our algorithm differs from that of Ta-Shma and uses a combination of amplitude amplification and phase estimation.  This completely sidesteps both the somewhat complicated analysis and intermediate measurements of Ta-Shma's algorithm. 
