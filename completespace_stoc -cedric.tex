...
We first briefly summarize the algorithm of Ta-Shma \cite{tashma}, which  is based on the linear systems solver of Harrow, Hassidim and Lloyd \cite{HHL}.  In our language, Ta-Shma shows that $\matrixinvert{\bigoh(\log{n})}$ can be solved by a quantum logspace algorithm that uses intermediate measurements. This corresponds to inverting an $n \times n$ matrix with condition number at most $\poly(n)$.
